...
if ! ip route get ${DESTINATION[0]} | grep -q macvlan0; then
    ip addr add ${EGRESS_SOURCE}/32 dev macvlan0
    ip link set up dev macvlan0
    ip route add ${EGRESS_GATEWAY}/32 dev macvlan0
    # Hack to allow multiple egress destinations
    # Not sure if this is the best option but need a workaround for the time being
    COUNT=${#DESTINATION[@]}
    if [[ "${DESTINATION[0]}" != "${EGRESS_GATEWAY}" && $COUNT -ge 1 ]]; then
          for DEST_IP in ${DESTINATION[@]};
          do ip route add "${DEST_IP}"/32 via "${EGRESS_GATEWAY}" dev macvlan0;
              iptables -t nat -A PREROUTING -p tcp -m statistic --mode nth --every "${COUNT}" --packet 0 -i eth0 -j DNAT --to-destination "${DEST_IP}";
              ((COUNT--));
          done
    fi
